...
local schema_categories = {
  ...
  },
  {
    ...
    list_sql_mssql    = "select SCHEMA_NAME(schema_id) as ROUTINE_SCHEMA, name as ROUTINE_NAME, 'FUNCTION' as ROUTINE_TYPE from sys.objects where type in ('FN','IF','TF') order by SCHEMA_NAME(schema_id), name",
    list_sql_sqlite   = nil,
  },
  {
    key = "procedures",
    ...
    list_sql_mssql    = "select SCHEMA_NAME(schema_id) as ROUTINE_SCHEMA, name as ROUTINE_NAME, 'PROCEDURE' as ROUTINE_TYPE from sys.procedures order by SCHEMA_NAME(schema_id), name",
    list_sql_sqlite   = nil,
  },
}
...
-- ============================================================
-- Procedure/Function Editing (Feature: proc-edit)
-- ============================================================
...
local function save_procedure_definition(buf)
  ...
  local meta = vim.b[buf].sqlui_procedure
  
  if not meta then
    notify("Metadados de procedure nao encontrados", vim.log.levels.ERROR)
    return false
  end
  
  -- Get current buffer content (the edited definition)
  local lines = vim.api.nvim_buf_get_lines(buf, 0, -1, false)
  local new_definition = table.concat(lines, "\n")
  
  if trim(new_definition) == "" then
    notify("Definicao nao pode estar vazia", vim.log.levels.WARN)
    return false
  end
  
  if trim(new_definition) == trim(meta.original_definition) then
    notify("Nenhuma mudanca detectada", vim.log.levels.INFO)
    return true
  end
  
  -- Generate ALTER statement
  local alter_sql = generate_alter_procedure_sql(
    meta.driver,
    meta.schema,
    meta.name,
    new_definition,
  ...
  -- Execute ALTER via usql
  local result = vim.system(
    { usql_bin(), "-c", alter_sql, meta.conn.dsn },
    { text = true }
  ):wait()
  ...
  if result.code ~= 0 then
    ...
    notify("Erro ao salvar procedure: " .. (err ~= "" and err or "Erro desconhecido"), vim.log.levels.ERROR)
    return false
  end
  
  -- Success: update buffer state
  meta.original_definition = new_definition
  vim.b[buf].sqlui_procedure = meta
  vim.bo[buf].modified = false
  
  notify("Procedure " .. meta.schema .. "." .. meta.name .. " salva com sucesso")
  return true
end

local function setup_procedure_buffer(buf, conn, item, driver)
  -- Store metadata in buffer for later save operation
  vim.b[buf].sqlui_procedure = {
    schema = item.schema,
    name = item.name,
    type = item.type or "PROCEDURE",
    driver = driver,
    conn = conn,
    original_definition = table.concat(vim.api.nvim_buf_get_lines(buf, 0, -1, false), "\n"),
  }
  
  -- Create :w command override that saves to database
  vim.api.nvim_buf_create_user_command(buf, "SqlUiProcedureSave", function()
    save_procedure_definition(buf)
  end, { desc = "Save procedure/function changes to database" })
  
  -- Hook BufWriteCmd so :w also saves to database
  vim.api.nvim_create_autocmd("BufWriteCmd", {
    buffer = buf,
    callback = function()
      save_procedure_definition(buf)
    end,
    desc = "Auto-save procedure/function to database on :w"
  })
end
...
local function open_routine_sql(conn, item)
  ...
  -- Feature: Procedure Editing - enable editing and save
  setup_procedure_buffer(buf, conn, item, driver)
end
